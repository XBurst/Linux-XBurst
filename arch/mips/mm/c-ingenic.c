						     ...
						     unsigned long addr, unsigned long pfn)
{
	...
	if (mips_machtype >= MACH_INGENIC_X2000)
		for_each_cpu(cpu, cpu_present_mask)
			valid_asid |= cpu_context(cpu, vma->vm_mm);
	else
		valid_asid = cpu_context(smp_processor_id(), vma->vm_mm);
...
if ((!(pte_present(*ptep))) != (!(pte_val(*ptep) & _PAGE_PRESENT))) {
	while(1)
		printk("-----%d-----%d\n",!(pte_present(*ptep)),!(pte_val(*ptep) & _PAGE_PRESENT));
}
