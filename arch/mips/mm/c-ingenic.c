...
static void xburst_dma_cache_inv(unsigned long addr, unsigned long size)
{
	...
	if (!ingenic_op_needs_ipi() && size >= dcache_size)
		/*ingenic_inv_dcache();*/ingenic_blast_dcache();
	else
...
void ingenic_cache_init(void)
{
	...
	__flush_cache_vmap = ingenic_cache_noop;
	__flush_cache_vunmap = ingenic_cache_noop;
	__flush_cache_all = ingenic_cache_noop;
	...
	__flush_kernel_vmap_range = (void *)ingenic_cache_noop;

	flush_cache_range = (void *)ingenic_cache_noop;
	flush_cache_page = (void *)ingenic_cache_noop;
	...
	local_flush_data_cache_page = (void *)ingenic_cache_noop;
	...
	if (current_cpu_type() == CPU_XBURST) {
		...
		if (mips_machtype >= MACH_INGENIC_X1830)
			...
			_dma_cache_wback = xburst_dma_cache_wback_inv;
