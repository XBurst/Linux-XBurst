...
static int ingenic_mac_set_mode(struct plat_stmmacenet_data *plat_dat)
{
	...
	case PHY_INTERFACE_MODE_RGMII:
		if ((mac->soc_info->version < ID_X2000) &&
			(mac->soc_info->version > ID_JZ4775))
			goto unsupported_interface;

		val = (1 << 31) | (1 << 19) | (62 << 12) | (1 << 0);
...
static struct ingenic_soc_info x2000_soc_info = {
	.version = ID_X2000,
	.mask = MACPHYC_TX_SEL_MASK | MACPHYC_TX_DELAY_MASK | MACPHYC_PHY_INFT_MASK,
