...
static int ingenic_sfc_exec_op(struct spi_mem *mem, const struct spi_mem_op *op)
{
	...
	u32p_replace_bits(&val,
			...
			GLB_TRAN_DIR_MASK);
	if ((op->data.dir == SPI_MEM_DATA_IN && IS_ALIGNED(virt_to_phys(op->data.buf.in), 4)) ||
		(op->data.dir == SPI_MEM_DATA_OUT && IS_ALIGNED(virt_to_phys(op->data.buf.out), 4)))
	...
	if (op->data.dir == SPI_MEM_DATA_OUT && !IS_ALIGNED(virt_to_phys(op->data.buf.out), 4))
