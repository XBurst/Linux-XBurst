...
#define CPM_REG_USBPCR				0x3c/*0x00*/
#define CPM_REG_USBRDT				0x40/*0x04*/
#define CPM_REG_USBVBFIL			0x44/*0x08*/
#define CPM_REG_USBPCR1				0x48/*0x0c*/
...
static int jz4770_phy_init(struct usb_phy *phy)
{
	...
	if (priv->version >= ID_X1830) {
		...
		reg = USBPCR_IDPULLUP_OTG | USBPCR_VBUSVLDEXT |
			USBPCR_VBUSVLDEXTSEL | USBPCR_TXPREEMPHTUNE;
	} else if (priv->version >= ID_X1000) {
		reg = readl(priv->base + CPM_REG_USBPCR1) &
			~USBPCR1_REFCLKDIV_MASK;
		reg = USBPCR1_REFCLKSEL_CLKCORE | USBPCR1_REFCLKDIV_24M |
