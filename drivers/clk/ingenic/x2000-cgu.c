		       ...
		       unsigned int *pm, unsigned int *pn, unsigned int *pod)
{
	 ...
	 * The highest divider yields the best resolution.
	 */
	od = round_down(4800 * MHZ / rate, 2);

	/* The frequency of the VCO must be at least 2400 MHz. */
	od = clamp_val(od, round_up(2400 * MHZ / rate, 2), pll_info->od_max);
...
static int x2000_cpu_set_rate(struct clk_hw *hw, unsigned long req_rate, unsigned long parent_rate)
{
	...
	cpccr = readl(cgu->base + CGU_REG_CPCCR) & ~0xff;
