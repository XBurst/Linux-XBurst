...
static unsigned long jz4775_cpu_recalc_rate(struct clk_hw *hw, unsigned long parent_rate)
{
	return parent_rate / ((readl(cgu->base + CGU_REG_CPCCR) & CPCCR_CDIV_MASK) + 1);
}

static long jz4775_cpu_round_rate(struct clk_hw *hw, unsigned long req_rate,
				      unsigned long *parent_rate)
{
	return *parent_rate / ((*parent_rate - 1) / req_rate + 1);
}

static int jz4775_cpu_set_rate(struct clk_hw *hw, unsigned long req_rate, unsigned long parent_rate)
{
	unsigned int div;
	u32 cpccr;
...
static const struct clk_ops jz4775_cpu_ops = {
	.recalc_rate = jz4775_cpu_recalc_rate,
	.round_rate = jz4775_cpu_round_rate,
	.set_rate = jz4775_cpu_set_rate,
};
...
static const struct ingenic_cgu_clk_info jz4775_cgu_clocks[] = {
	...
	[JZ4775_CLK_CPU] = {
		...
		.parents = { JZ4775_CLK_CPUMUX, -1, -1, -1 },
		.custom = { &jz4775_cpu_ops },
	},
